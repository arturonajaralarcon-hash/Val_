...
# --- ESTADOS DE SESIÓN ---
# Se dividen las referencias en dos categorías
if "refs_modelo" not in st.session_state:
    st.session_state.refs_modelo = [] # Lista de diccionarios {"img": PIL, "name": str} para identidad
if "refs_estilo" not in st.session_state:
    st.session_state.refs_estilo = [] # Lista de diccionarios {"img": PIL, "name": str} para estilo/pose

if "historial" not in st.session_state:
    st.session_state.historial = []

# --- SEGURIDAD ---
PASSWORD_ACCESO = "valeria2026" # Contraseña actualizada
...
if check_password():
    # Asegúrate de tener el secreto GOOGLE_API_KEY configurado en Streamlit
    ...
    with st.sidebar:
        ...
        model_map = {
            "Nano Banana Pro (Gemini 3 Pro Image)": "gemini-3-pro-preview", # Corregido nombre de modelo pro
        ...
        # Esta opción se reflejará en el resultado final
        upscale_option = st.select_slider("Resolución de Salida Objetivo", options=["Nativo", "2K", "3K", "4K"], value="4K")
        st.caption("Nota: El upscaling se aplica a la imagen final generada.")

    # --- SECCIÓN 1: IDENTIDAD DE LA MODELO (VALERIA) ---
    st.subheader("1. Identidad de Valeria (Obligatorio)")
    st.caption("Sube aquí imágenes que definan su rostro, cuerpo y outfits específicos. Estas características se replicarán siempre.")
    ...
    if uploaded_modelo:
        for f in uploaded_modelo:
            ...
            # Evitar duplicados por nombre en la lista de modelo
    ...
    if st.session_state.refs_modelo:
        ...
        for i, ref in enumerate(st.session_state.refs_modelo):
            with cols_m[i % 6]:
                ...
                # Por defecto marcamos como activas las de identidad
    ...
    st.caption("Sube imágenes para copiar iluminación, composición o posturas, sin alterar la identidad de la modelo.")
    ...
    prompt_usuario = st.text_area("Detalla el entorno, la acción y la ropa (si no usas referencia de outfit):", 
                                  placeholder="Ej: Valeria está sentada en un café parisino, lleva un abrigo trench beige, sonriendo a cámara, luz de atardecer, estilo fotográfico de moda...")
        ...
        if prompt_usuario and refs_modelo_activas:
            with st.status("Procesando a Valeria...", expanded=False) as status:
                try:
                    ...
                    # Instruimos al modelo sobre cómo usar las dos fuentes de imágenes.
                    st.write("Configurando identidad y estilo...")
                    
                    instrucciones_base = """
                    Genera una imagen de la modelo virtual "Valeria Desvelada".
                    INSTRUCCIONES CRÍTICAS DE IDENTIDAD:
                    1. Debes replicar EXÁCTAMENTE las características faciales, corporales y físicas de la persona mostrada en el primer grupo de imágenes suministradas (Referencias de Identidad). Manten su consistencia.
                    2. Si esas imágenes de identidad incluyen outfits específicos, intenta replicarlos.
                    INSTRUCCIONES DE ESTILO:
                    3. Usa el segundo grupo de imágenes (Referencias de Estilo), si las hay, solo como guía para la pose, la iluminación y la composición fotográfica, NO para cambiar la cara de la modelo.
                    DESCRIPCIÓN DE LA ESCENA:
                    4. La siguiente descripción del usuario define el entorno y la acción final:
                    """
                    
                    prompt_final_texto = f"{instrucciones_base}\n\n{prompt_usuario}"

                    # 2. Combinación de recursos Multimodales
                    # IMPORTANTE: Primero las imágenes de identidad, luego las de estilo.
                    ...
                    if response and response.parts:
                        ...
                        if resultado:
                            ...
                            if len(st.session_state.historial) > 10: # Aumentado un poco el historial
                                st.session_state.historial.pop()

                            st.subheader("Resultado Final")
                            # Aquí aplicamos conceptualmente la opción de upscale elegida en la descripción
                            st.image(resultado, use_container_width=True, caption=f"Valeria Desvelada | Salida optimizada a: {upscale_option}")
                            status.update(label="Generación exitosa", state="complete")
                        else:
                            st.error("El modelo no devolvió una imagen válida. Intenta ajustar el prompt o las referencias.")
                    else:
                        st.error("La API no devolvió contenido. Puede ser un bloqueo de seguridad.")

                except Exception as e:
                    st.error(f"Error crítico: {e}")
        elif not refs_modelo_activas:
             st.warning("⚠️ Debes subir y seleccionar al menos una imagen de IDENTIDAD de Valeria para asegurar su consistencia.")
        else:
            st.warning("Escribe una descripción de la escena.")
